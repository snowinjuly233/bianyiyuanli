...
std::filesystem::path derive_project_root(const std::string& yacc_spec_path) {
    const auto absolute_spec = std::filesystem::absolute(std::filesystem::path(yacc_spec_path)).lexically_normal();
    const auto spec_parent = absolute_spec.parent_path();
    if (spec_parent.filename() == "specs" && spec_parent.has_parent_path()) {
        return spec_parent.parent_path();
    }
...
std::vector<compilerlab::common::Token> scan_tokens(const std::string& lex_spec_path,
    ...
    while (true) {
        ...
        if (token.kind == compilerlab::common::TokenKind::Invalid) {
            diagnostics.error("invalid token `" + compilerlab::common::escape_text(token.lexeme) + "`", token.span);
            return {};
        }
...
int run(int argc, char** argv) {
    const std::string yacc_spec_path = argc > 1 ? argv[1] : "specs/minic.y";
    const std::string input_path = argc > 2 ? argv[2] : "specs/samples/minic_demo.c";
    const std::string lex_spec_path = argc > 3 ? argv[3] : "specs/minic.l";
    const std::string output_dir = argc > 4 ? argv[4] : "generated";
    ...
    compilerlab::seuyacc::ParserEmitter parser_emitter;
    ...
    const auto parser_name = std::filesystem::path(yacc_spec_path).stem().string();
    const auto scanner_name = std::filesystem::path(lex_spec_path).stem().string();
    const auto project_root = derive_project_root(yacc_spec_path);
    if (!parser_emitter.emit(grammar_bundle.grammar, parse_table, grammar_bundle.productions,
                             grammar_bundle.token_symbols, output_dir, parser_name, scanner_name,
                             lr1_automaton.states.size(), lalr_automaton.states.size(),
                             project_root.string(), diagnostics)) {
        print_diagnostics(diagnostics);
        return 1;
    }
    ...
    const auto semantic_result = parser_runtime.parse(
        ...
        [&](int production_index, const std::vector<compilerlab::semantic::SemanticValue>& rhs_values) {
            if (production_index > 0 &&
                ...
                ++reduce_step;
                std::cout << "reduce " << reduce_step << ": "
                          << grammar_bundle.productions[static_cast<std::size_t>(production_index)].debug_name
                          << '\n';
            }
    ...
    std::cout << "ACCEPT\n";
    std::cout << "Generated parser: "
              << (std::filesystem::path(output_dir) /
                  (parser_name + "_parser.h")).lexically_normal().string()
              << '\n';
    std::cout << "Generated parser: "
              << (std::filesystem::path(output_dir) /
                  (parser_name + "_parser.cpp")).lexically_normal().string()
              << '\n';
    std::cout << "Generated parser: "
              << (std::filesystem::path(output_dir) /
                  (parser_name + "_parser_main.cpp")).lexically_normal().string()
              << '\n';
